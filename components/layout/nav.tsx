...
export const Nav = ({ data }: { data: GlobalHeader }) => {
  ...
  return (
    <div
      className={`relative overflow-hidden bg-wvu-accent--blue-dark`}
    >
      <Container size="custom" className="py-0 relative z-10">
        <ul className="flex gap-6 sm:gap-6 tracking-[.002em]">
          {data.nav &&
            data.nav.map((item, i) => {
              const activeItem =
                (item.href === ""
              ...
              return (
                ...
                >
                  <Link
                    ...
                    href={`/${item.href}`}
                    className={`relative select-none text-white inline-block tracking-wide transition duration-150 ease-out hover:opacity-100 py-4 px-2 ${
